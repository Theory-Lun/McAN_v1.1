...
//     The mutation format:
//         <SampleName>\t<SampleID>\t[<POS>(<VariantType>:<REF>-><ALT>);]...
//---------------------------------------------------------------------------
...
extern "C" {
    int vcf2mut(const char* str_vcf, const char* str_mut, Callback lineHandler, void* arg);
}
...
int vcf2mut(const char* str_vcf, const char* str_mut, Callback lineHandler, void* arg) {
    std::ifstream input;
    if (str_vcf[0] != '-' || str_vcf[1] != 0) {
        input.open(str_vcf);
        if (!input.good()) {
            std::cerr << "\nError: failed to open file: " << str_vcf << '\n';
            return -1;
        }
    }
    std::istream& in = (str_vcf[0] != '-' || str_vcf[1] != 0) ? input : std::cin;
    ...
    if (str_mut) {
        ...
        out.open(str_mut);
        if (!out.good()) {
            std::cerr << "\nError: failed to create file: " << str_mut << '\n';
            return -1;
        }
